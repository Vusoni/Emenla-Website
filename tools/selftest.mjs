...
for (let i = 0; i < bands.length; i++) {
  ...
  for (const prog of probes) {
    ...
    const c = kinds[i] === 'violet' ? Math.min(contrast(0.225, maxL), contrast(0.03, minL)) : contrast(0, minL);
...
/* ---------- Video blocked ---------- */
const nb = await b.page(URL, { width: 1440, height: 900, block: ['*hero-scrub.mp4'] });
